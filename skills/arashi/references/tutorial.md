...
Use the paths reported by `aw status` to enter the created worktrees and run each project's existing validation there. Use `aw exec` only when child repositories are already configured, and reuse the same real `--group` or `--only` selector that was inspected earlier; do not invent an example repository name.

Exact init/create semantics are in [Workspace and repositories](commands/workspace.md) and [Create worktrees](commands/create.md). Installed `aw <command> --help` is the option authority.
...
- original uncommitted changes remain where they started unless `aw move` was deliberately used;
- `aw status` reports the intended branch/worktree state; and
