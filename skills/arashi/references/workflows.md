...
4. Create one coordinated branch without user-facing launch during unattended work:
   ...
   ```bash
   aw create feature/skill-integration --no-launch --no-switch
   ```
...
5. Verify the resulting worktrees with `aw status`, then use the paths reported by `aw status` to run project-local validation. When configured child repositories exist, `aw exec` may run repeated non-interactive validation with the same selector scope.
...
1. Run `aw status` or `aw doctor --json`.
2. Use paths reported by `aw status` for project-local validation. When configured child repositories exist, `aw exec` may run repeated validation with the same inspected selector.
3. Record created/reused/skipped/failed repositories and any preserved worktrees.
4. Use `aw handoff` when a structured workspace report is useful.
