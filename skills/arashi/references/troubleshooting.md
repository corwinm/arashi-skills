...
| `arashi --version` exits immediately or returns `137` | Installed binary is invalid for the current platform or a published release artifact is bad | Reinstall using a pinned version from the website guide, verify `arashi --version`, and report the bad release artifact. |
