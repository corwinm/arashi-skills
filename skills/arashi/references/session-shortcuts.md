...
aw switch --path "/exact/reviewed/worktree/path"
```

Default `aw list` output is the pipe-friendly path surface, so this composition requires only the conditional `fzf` integration. Keep selection and execution separate, quote the reviewed path, and never evaluate it as shell code.
