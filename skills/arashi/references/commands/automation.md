...
- `aw doctor --json` reports managed-ignore problems as stable diagnostic findings rather than repairing them.
...
Use `aw handoff` when an agent needs to pause, switch with another worker, request review, or leave dirty coordinated work with explicit context.

```bash
RELATED_URL="https://example.com/review"
NEXT_CHECK_COMMAND="aw status"
...
aw handoff --json --link "$RELATED_URL"
```
...
For `aw pull`, leave children serial by default (`--jobs 1`); use a positive safe integer with `--jobs <n>` only to bound independent child pulls. Parent pull, configuration reload, and managed-ignore reconciliation finish first. Uncertain Git identity falls back to serial execution; results keep configured order. A failed child stops new starts but already-started pulls finish. See the canonical [pull command reference](https://arashi.haphazard.dev/commands/pull/) for selection and failure behavior.
...
Use `aw push` after committing implementation changes and before opening cross-repo PRs.
...
- Prefer `--json` when an agent or script needs to parse per-repository stdout, stderr, child exit status, duration, selected repositories, or aggregate totals. Any child command failure makes the `aw exec` process exit non-zero.
