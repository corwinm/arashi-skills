...
The repository-level `contracts/command-coverage.json` file is synchronized with the CLI's top-level command contract. Every command must have exactly one entry: use `covered` with a valid reference relative to `skills/arashi/`, or `excluded` with a stable, non-empty `reason`. Classify standalone behavior as `supported`, `configured-only`, `conditional`, or `not-applicable`, and include a reason unless it is supported directly. The conditional `init --zero-config` entry also records its complete dry-run, JSON, compatible-option, and incompatible-option policy. The manifest is intentionally outside `skills/` so installed skills do not ship maintainer-only validation metadata, and it remains separate from the curated prose in `references/`; do not turn the prose into an exhaustive generated command catalog.
