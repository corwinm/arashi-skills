...
const requirements = new Map([
  [
    ...
    [
      ...
      "`defaults.create` remains unchanged",
...
function validateSkill(root, label) {
  ...
  assert.match(
    commands,
    /"create"\s*:\s*\{[^}]*"launchMode"\s*:\s*"herdr"/s,
    `${label} must keep defaults.create launchMode unchanged`
  );
}
...
function main() {
  ...
  const packageRoot = mkdtempSync(join(tmpdir(), "arashi-switch-skill-package-"));
  try {
